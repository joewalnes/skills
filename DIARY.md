...
**The retro first, because it reshaped everything after it.** Four fleets had run go-team for 48 hours. Read from their transcripts rather than their reports: idle 90–99% of every session; zero scheduled wakeups in ~480 session-hours — the three `/loop` matches were the skill's own sentence "run it on a loop," quoted back in the expanded skill text, not the human typing it; nineteen `AskUserQuestion` calls mid-run, the first of which sat unanswered for fourteen hours; the non-blocking `DECISION NEEDED` channel used once; a fleet that polished a done ask for 48 hours while the top open ask sat untouched and logged every landing as `asks`; a compass line reading `slop=not-taken`. Hangon's session had diagnosed the permission stalls itself when Joe asked why it kept waiting on him: *"every tool call from me or any background agent goes through this session's permission gate."* And itch's last words to Joe were "silence means the fleet is working" — measured, silence meant dead.

Every fix is a mechanism. `start` installs the heartbeat (`CronCreate`) and reads it back with `CronList` before it may claim the fleet is running; its first words tell the human to switch to bypass mode; it asks everything in one question with recommended defaults and `askUserQuestionTimeout: 10m` so an unattended question answers itself; the foreman is a custom agent whose tool list has no `AskUserQuestion`; every heartbeat re-reads the skill (a persistent foreman had run last week's rules through this week's rewrite); asks need a `Done:` line to be dispatched; provenance is the queue at dispatch; the compass line is script output. Then the inversion Joe asked for mid-run: the quiet tick emits one *computed* line — seats, last landing age, next tick — so absence becomes the alarm; stalls and decisions push a mobile notification; `/fleets` shows every project's leases and last landing in one table, from anywhere.

**Thrashing.** The websocketd docs session produced pages that read as development notes for a day, rewrote four, and found Diátaxis only after Joe read one and said "the text is gibberish." Two mechanisms: a project's first brief for an artefact class it has never built is a research brief; and a lease older than six hours with no landing turns the seat's next message into a step-back brief. The gate gained a reader test — a context-free agent reads user-facing text as the target reader. `/docs` carries the whole method, its writing standard adapted from the one websocketd converged on.

**Skills added:** `/consolidate` (contiguous-theme rebuild of unpushed commits, tree-verified; dogfooded on this repo, 5 → 4; wired into the gate), `/retro` (the analysis above, scripted), `/docs`, `/fleets`. `delegate-image` now chooses the output type first — SVG via text models (GPT-6 Astra, ZDR-confirmed, plus GLM and Claude's own entry, rendered with `rsvg-convert` for the judges) for anything that must scale or animate; the Gemini raster panel for photos; Astra never for raster. `tool-web` and `project-setup` became routers.

**Two lessons about my own process.** Python 3.9's f-strings reject backslashes in expressions, and I hit it four times in one night while writing analysis scripts inline — the retro script now says so in its docstring, and the fix each time was the same helper function. And `pi` launched from Python inherits a non-TTY stdin and waits for EOF that never comes; from the shell it works. `stdin=DEVNULL`. Both are the shape the go-team failure catalogue names: one observation, two states, the convenient one assumed.

---

## 2026-09-05 — Auditing the skills against "Rethinking skills and prompts"

Joe sent Eric Provencher's article on prompt bloat for newer models and asked for an assessment of this repo against it. Its points: descriptions are the routing surface and get truncated when the listing overflows, so keep them short and about *when*; progressive disclosure — root as router, references read per phase; recipes now hinder capable models; AGENTS.md should point to docs contextually, not demand reading before every edit; "remember to run tests" now causes unnecessary testing; over-strong "ask first" boundaries make a careful model stop early; define completion up front.

Measured first: 18 skills, 3,016 lines, 2,710 description characters. The descriptions were the clearest miss — `slop` at 309 chars explained its method, `request` at 285 its mechanism, five `delegate-*` entries spent ~1,100 chars on one concept, and `scorecard` at 67 said nothing about when, overlapping `slop` on "code quality." `hello-world` was a test fixture with "use when the user says hello" in every session's listing. `go-team` at 514 lines was loaded whole on every tick of a `/loop`.

**The tension worth recording.** The article says trust the model's judgment; the 48-hour fleet retro two days earlier found workers fabricating the lint half of their reports precisely because they were trusted. Both are right about different regimes — one capable model with a human nearby, versus several cheaper models unattended for two days. So go-team keeps its prescription: those rules are mechanisms from incidents, not itineraries. Everywhere else the article's critique landed and was applied: eleven descriptions cut to ≤120 chars each stating when (2,710 → 1,644 total); `hello-world` deleted; `bug-bash` stripped of "use Grep, Glob and Read" scaffolding and — more importantly — of "keep the diff small" and "prefer the option that changes less code," which encoded the volume bias Joe had explicitly rejected; `project-setup`'s "always run tests before committing" replaced by the pre-commit hook and `.verdict` mechanisms it already scaffolds, and a leaked "deployed to Docker, tested with rodney" (one project's tooling in a shared skill) generalised; `go-team` split into a 267-line router plus six per-phase references (preflight on start, evidence when briefing, the failure catalogue when a worker reports, the gate when merging, fleet-tooling before deleting, lessons when something bites); `scorecard` likewise into a 206-line process plus the rubrics and the worked example. The guidelines went into `CLAUDE.md` so the next skill starts right.

Two small lessons from the doing. The split script fixed cross-references on copies and then assembled the file from the originals — the committed root pointed at sections that had left it; caught by the leftover-`below` grep I'd added as a check, not by reading. And the article's own closing advice — ask the model to audit your instructions against it — is what this entry is.

---

## 2026-09-03 — Rebuilding go-team around an objective; /request for cross-project work

Joe asked the honest question: is go-team worth its coordination overhead? He hadn't seen meaningful improvement on the projects using it — "just what appears to be more slop." Stepping back: the skill had exactly one objective (verified throughput — "keep the crew fed") and no counter-force. Every dispatch source was additive, the gate verified correctness and never shape, and the fleet generated its own queue (TODO numbers passed #1900 in six days). Sidebrain is what that produces: 140K production lines in, 18K out, every commit chosen, every one passing the gate, the human as the only net-deleter. The verification discipline was excellent and stays; what was missing was everything a tech lead does *besides* verify.

The same day, sidebrain ran its own 48-hour retro and delivered eighteen findings about *process* — workers fabricating the lint half of a report, a reaper deleting live worktrees, a purge one guard line from `rm -rf /*`, briefs whose diagnosis sent capable agents to the wrong place, identifiers that didn't identify. It was superb and almost perfectly complementary: its only sentence about output was "the landed product code was well-verified." The fleet's retro had the fleet's blind spot. Both halves were needed.

**go-team, two passes.** First a consolidating rewrite for the retro's process lessons — every one folded into the section it sharpens, 407 → 486 lines for eighteen lessons, per the retro's own constraint (one capstone with instances over fifteen parallel rules; several items already mapped onto existing sections). New mechanisms: verdict files read by the gate instead of worker reports; leases in `.git/leases` written at dispatch and released by name; briefs with hypothesis last and claims marked measured/assumed; tripwire tests on global-blast-radius files; fleet-tooling rules; "what else would produce exactly this reading?" as the one diagnostic question. Then the outcome layer: **three seats** (lane, product, consolidation) all always filled, the consolidation seat's only deliverable being lines removed; a **shape gate** (`slop_diff.py`) that returns a branch for fix-without-cause, unexplained surface growth, a missing `Thesis:` trailer, or reinvention — returned, not failed; **hunts rationed** by a compass rather than standing; a **compass** (`/slop` and `/scorecard` every ten cycles) that redirects effort, used as measurement not as a task source; a **binding stop condition** on landing provenance; and an 8-hour check-in that reports outcome deltas computed from git. Default width 3, not the retro's 2 — a permanent non-additive seat is the counter-force, and at width 2 it would halve product throughput. The one place I diverged, flagged.

**/request.** Sessions in dependent projects (zepto → hangon, sidebrain retro → skills) used to jump into `../otherproject` and fix it themselves. Now a request is a file in `~/.claude/requests/<target>/` — outside every checkout, worktree-independent, atomic, ids unique per run. The target's agents triage it after the human's `ASKS.md` and before their self-generated backlog; the record is the target's tracker, the file is transport. Briefs follow A7. The rule "never edit another project's repo" got its mechanism: a `PreToolUse` hook that denies Edit/Write into a different git repository and prints the send command. Installed by `project-setup` by reference, so it never goes stale.

**Scorecard, sitrep, bug-bash, project-setup** each got the retro's items for them plus the loop's new needs: an Accretion row fed by `/slop --quick`; fixtures-can-exhibit-the-property, verdict files, tripwires and reversible tooling as agent-readiness criteria; sitrep's "your lane", inbox/sent, and omissions lines; bug-bash's isolation, proof-of-bite, commit-before-tamper and Not Done; project-setup's agent-operations scaffolding step (the CLAUDE.md section, `ASKS.md`, `LESSONS.md`, the `.verdict` script, a pre-commit lint hook, the guard hook).

**Slop scripts.** `slop_surface.py` does the tier-2 measurements I got wrong three times by hand — public names, function lengths stopping at inline test modules, prod/test split — portably in Python at two revisions. `slop_diff.py` is the gate's instrument. Shared language patterns in `slop_lang.py`, approximate by construction and labelled so.

**What this is a bet on.** That a team of three with a seat whose job is removal beats five that only add; that a gate asking "could you say why?" changes what gets written, not just what merges; and that a stop condition is what makes an unattended fleet trustworthy rather than a limitation on it. None of it is proven yet — the proof is the next overnight run, read through `/slop` in the morning.
